...
# Objectives  
I will be importing eca data that contains the elevation data. From this I will interpolate elevation, and then derive other terrain variables, make maps, and export all layers to file.
...
elev = read_sf("../data/50ac/ec/VSEC_50_acre.shp") %>%
  clean_names() %>%
  mutate(elev_m = elevation) %>%
  dplyr::select(elev_m) %>%
  st_transform(6345)
