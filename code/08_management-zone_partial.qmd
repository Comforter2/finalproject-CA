...
```{r sty_v}
sty_v  = read_sf("../../07_yieldmonitor/data/sy_all.geojson")
...
```{r eca_v }
eca_v = read_sf("../../09_soilec/data/eca_v.geojson")
...
```{r all_v}
all_v = read_sf("../../10_stabilityvalidation/data/all_v.geojson")
...
If you have a y-variable that is the predicted variable, and you provide the y variable, it is supervised analysis. But analysis that are unsupervised do not take y variables, but onmly x variables. You are not giving the labels, and the analysis is suppossed to find the commonality.
...
ggsave("../output/zonesmoothed_5x5_mean2.png",
       width = 3, 
       height = 4)
```
