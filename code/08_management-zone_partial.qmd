...
```{r sy anova and letter separation}
library(emmeans)
library(multcomp)
library(purrr)
library(car)
...
zone_s_df %>%
  ...
  ggplot(aes(x=cluster_f, y = value))+
  geom_boxplot(aes(fill = cluster_f))+
  facet_wrap(~name, scales = "free_y",
             labeller = as_labeller(c(sy16c = "2016 Corn",
                                      sy17s = "2017 Soybeans",
                                      sy23c = "2023 Cotton",
                                      sy24c = "2024 Cotton"))) +
  geom_label(data = anovas_sy,
             aes(x = cluster_f,
                 y = emmean,
                 label = letter))+
  ...
  labs(x = "Cluster", y = "Standardized Yield") 

ggsave("../output/sy_validate.png",
       width = 10,
       height = 6)
...
```{r soil-terrain anova and letter separation}
anovas_soil_terrain <- zone_s_df %>%
  dplyr::select(-ph_class, -plbac_class, -slope) %>%
  pivot_longer(cols = elev_m:k_lbac) %>%
  group_by(name) %>%
  nest() %>%
  mutate(mod = map(data,
...
```{r soil-terrain boxplot and labels}
zone_s_df %>%
  dplyr::select(-ph_class, -plbac_class, -slope) %>%
  pivot_longer(cols = elev_m:k_lbac) %>%
  mutate(name = factor(name,
  ...
  geom_boxplot(aes(fill = cluster_f))+
  facet_wrap(~name, scales = "free_y",
             labeller = as_labeller(c(elev_m = "Elevation (m)",
                                      ph = "Soil pH",
                                      p_lbac = "Phosphorus (lb/ac)",
                                      k_lbac = "Potassium (lb/ac)",
                                      eca30_dsm = "ECa shallow",
                                      eca90_dsm = "ECa deep"))) +
  geom_label(data = anovas_soil_terrain,
             aes(x = cluster_f,
                 y = emmean,
                 label = letter))+
  scale_fill_viridis_d(option="D")+
  ...
  labs(x = "Cluster", y = "Soil and Terrain Variables") 

ggsave("../output/soil-terrain_clust.png",
       width = 10,
       height = 6)
...
ggsave("../output/prop_stclass_clust.png",
       width = 7,
       height = 5)
